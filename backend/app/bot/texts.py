...
def vip_text() -> str:
    ...
    lines.append("\n💳 Оплата: криптовалюта, 🤖 Crypto Bot или Telegram Stars ⭐️")
    lines.append("Доступ выдаётся автоматически после подтверждения оплаты.")
    return "\n".join(lines)
...
def pocket_text() -> str:
    lines = [
        ...
        "Платите меньше — получаете аккаунт с балансом выше суммы оплаты:\n",
    ]
...
def payment_method_text(plan_name: str) -> str:
    return (
        ...
        "🤖 Crypto Bot или Telegram Stars ⭐️\n"
...
def cryptopay_invoice_text(plan_name: str, amount_usd: float) -> str:
    return (
        "🤖 <b>Счёт Crypto Bot создан!</b>\n\n"
        f"📦 {plan_name}\n💵 Сумма: <b>${amount_usd:.0f}</b>\n\n"
