...
- 💎 **VIP-подписки** (1/3/6 мес, lifetime) с **автовыдачей**: после оплаты бот сам
  шлёт одноразовый инвайт в VIP-канал, напоминает за 3 дня до конца и удаляет из
  канала по истечении
- 🏦 **PocketOption-заявки** — оплата автоматом, выдача аккаунта вручную из админки
- 💳 Оплата: крипта через **2328.io**, **Crypto Bot** (CryptoPay), **Telegram Stars**
...
backend/
  app/
    ...
    payments/        # шлюзы 2328.io / CryptoPay + watcher статусов
...
| `CRYPTOPAY_TOKEN` | @CryptoBot → Crypto Pay → API |
...
3. Готово: после оплаты бот шлёт одноразовый инвайт; фоновая джоба раз в час
   напоминает об окончании и убирает просроченных из канала

Если `VIP_CHANNEL_ID` пуст — автовыдача выключена, выдавай вручную из админки.
